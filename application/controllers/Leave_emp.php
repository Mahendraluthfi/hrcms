...
class Leave_emp extends CI_Controller {
	...
	public function index()
	{
		...
		foreach ($quota as $key) {
			$data['total'] = $key->employee_leave;
			$data['rem'] = $key->employee_leave_rem;
			$data['val_start'] = date('d M Y', strtotime($key->employee_start_leave));
			$data['val_end'] = date('d M Y', strtotime($key->employee_end_leave));
		}
		$row = $this->db->get_where('leaves', array('leave_employee' => $us_id, 'leave_category' => '1'))->result();
		$data['row'] = $row;
		$data['content'] = 'leave_emp';
...
/* End of file Leave_emp.php */
/* Location: ./application/controllers/Leave_emp.php */