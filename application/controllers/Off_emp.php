...
class Off_emp extends CI_Controller {
	...
	public function index()
	{
		...
		foreach ($quota as $key) {
			$data['total'] = $key->employee_off;
			$data['rem'] = $key->employee_off_rem;
			$data['val_start'] = date('d M Y', strtotime($key->employee_start_off));
			$data['val_end'] = date('d M Y', strtotime($key->employee_end_off));
		}
		$row = $this->db->get_where('leaves', array('leave_employee' => $us_id, 'leave_category' => '2'))->result();
		$data['row'] = $row;
		$data['content'] = 'off_emp';
...
/* End of file Off_emp.php */
/* Location: ./application/controllers/Off_emp.php */