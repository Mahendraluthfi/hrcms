...
class Salary_part extends CI_Controller {
	...
	public function view($id)
	{
		...
		for ($i=1; $i < 13; $i++) { 
			...
			if (empty($cek)) {
				$data['get'.$i] = '<a href="'.base_url('salary_part/createsession/').$id.'/'.$i.'" class="btn btn-danger btn-sm">Not Paid</a>';	
			}else{
	...
	public function createsession($id,$month)
	{
		...
		redirect('salary_part/generate','refresh');
	}
	...
	public function generate()
	{
		...
		$data['show'] = $this->db->get_where('employees', array('employee_id' => $id))->row();	
		$data['month'] = $month;			
		$data['content'] = 'salary_generate_part';
		$this->load->view('index', $data);				
	}
